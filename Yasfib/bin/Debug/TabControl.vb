' Part of the MDITabControl control. Written by Eduardo Oliveira, not me.
' In compliance with the GPL, I give him full credit for his control. To see the original project on CodeProject, visit:
...
#Region " Class TabPageCollection "
    ...
    Public Class TabPageCollection
        ...
        Private Sub TabPage_Clicked(ByVal sender As Object, ByVal e As EventArgs)
            If TabControl.FindForm().Equals(MainApplication) Then
                MainApplication.ChangeTab()
            End If
            ...
            End If
        End Sub
        Private Sub TabPage_Closed(ByVal sender As TabPage, ByVal e As EventArgs)
            Dim isTheOne As Boolean
            isTheOne = sender.Equals(MainApplication.TabControl1.TabPages.SelectedTab)
            Remove(sender)
            If TabControl.FindForm().Equals(DownloadManager) Then
                DownloadManager.ControlOverflow()
            End If
            If TabControl.FindForm().Equals(MainApplication) Then
                MainApplication.ChangeTab()
                If isTheOne = False Then
...
#End Region
    ...
    'Property TabAlignment As TabAlignment
    ...
    <Browsable(True), Category("Appearance"), Description("Gets or sets the System.Drawing.Color structure that represents the starting color of the Background linear gradient for the Hot tab button.")> _
    ...
    Private Sub CheckVisibility()
        ...
        Dim x As Integer = LeftOffset
        For i As Integer = 0 To Items.Count - 1
            If x + Items(i).Width < pnlControls.Left Then
                If Not Items(i).TabVisible Then
                    Items(i).TabVisible = True
                    Items(i).TabLeft = x
                    ShowTab(i)
                End If
    ...
    Private Sub pnlTop_Paint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles pnlTop.Paint
        ...
        Dim Pen As New Pen(RenderColors.BorderColor(m_RenderMode, BorderColor))
        If Alignment = TabAlignment.Top Then
